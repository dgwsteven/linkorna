...
function employeeBrief(employeeId: string) {
  ...
  const briefs: Record<string, string> = {
    ...
    meeting:
      "Meeting Recorder Employee. Its core job is to analyze uploaded meeting audio/video recordings that have been transcribed by the backend, then generate premium meeting minutes. Treat extractedText from uploaded meetingFiles as the meeting transcript. Optional notes or meeting context are supporting material only. Detail level must control depth. For Detailed minutes, write a complete meeting record with meeting background, participants if known, discussion flow, decisions, open questions, risks, assumptions, action owners, deadlines, dependencies and client-facing follow-up. If audio transcription failed or no meeting content is present, clearly explain what went wrong and ask for a supported recording instead of pretending to know the meeting content."
  };
...
export async function generateTaskOutput(employeeId: string, input: Record<string, unknown>): Promise<GeneratedTaskOutput> {
  ...
  const employeeSourceRule =
    employeeId === "meeting"
      ? "Meeting source rule: use transcribed audio/video extractedText from meetingFiles as the primary source. If transcription failed or no recording/transcript content is present, say so clearly and ask for a supported audio file rather than inventing minutes."
      : "";
